...
static bool Game3_EndCheck(void);
double frand3(void);
...
static int g_Game3ClearTimer = 0;
static bool g_Directing3Flg;

void Game3_Initialize(void)
{
	// ランダムシードの初期化
	srand((unsigned int)time(NULL));

	g_Game3ClearTimer = 0;
	g_Directing3Flg = false;
	g_NextSceneFlag = 0;
...
void Game3_Update(void)
{
	...
	case PHASE_INDEX_PLAYER_NORMAL:
		...
		if (g_Directing3Flg == false)
		{
			g_Game3ClearTimer++;
			if (g_Game3ClearTimer > 480)
			{
				g_Game3ClearTimer = 0;
				g_Directing3Flg = true;
			}
		}
		...
		if (Game3_EndCheck() && Player_IsEnable() == false)
		{
...
int GetGame3CleatTimer(void)
{
	return g_Game3ClearTimer;
}

bool GetGame3DirectingFlg()
{
	return g_Directing3Flg;
}